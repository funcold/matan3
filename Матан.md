...
### Доказательство
БОО $f_y'(x^0, y^0) > 0 => \exists O(x^0) : f'(x) > 0$
...
Производная = $f(x^0 + \Delta x^0, y^0 + \Delta y^0) - f(x^0, y^0) = 0$
...
Если $f_k(x, y)$ - непрерывна и дифференцируема в $\forall k f_k(x^0, y^0) = 0 O(x^0, y^0)$ и Якобиан не равен 0, то

$\exists g_1(x) ... g_n(x)$ - непрерывные, что $f_k(x_1, ..., x_m, g_1(x), ..., g_m(x)) = 0, \forall j = \overline{1, n}$

#### Доказательство

По индукции

База n = 1 - выполняется
...
$f(x) - f(x^0) = 1/2 \sum_{k=1}^m\sum_{j=1}^mf^{\prime\prime}_{x_kx_j}(\mathbf{x}^0) \Delta x_k \Delta x_j + o(||\Delta x ||^2)$
...
$= 1/2||\Delta x||^2\sum_{k=1}^m\sum_{j=1}^mf^{\prime\prime}_{x_kx_j}(\mathbf{x}^0)a_ka_j$
...
Тогда $x^0$ - локальный экстремум, если $\lambda_1, \lambda_2... \lambda_n : L(x_1, ... x_m, \lambda_1, ... \lambda_n) = f(x_1, ... x_m) - \sum_{k=1}^n \lambda_k g_k(x_1, ... x_m)$
...
По теореме о системах неявных функций $\exists \gamma_1(x_{n+1}, ... x_m), ... \gamma_n(x_{n+1}, ... x_m)$ такие, что $\forall k = \overline{1, n} g_k(\gamma_1... \gamma_n, x_{n+1}, ... x_m)$

$dh(x^0) = 0$ - необходимое условие экстремума, все частные производные = 0

$dh(x^0) = \sum_{k=n+1}^mh_{x_k}'dx_k =\sum_{k=1}^mf_k'dx_k = 0$ по инфариантности первого дифференциала

Подставим в g $\gamma$ функции и продифференцируем
...
$L_{x_j}' = \frac{\partial f}{\partial x_j} = \sum_{l=1}^n \lambda_l \frac{\partial g_k}{\partial x_j}, \forall j = \overline{1, n}$ - множитель Лагранжа
